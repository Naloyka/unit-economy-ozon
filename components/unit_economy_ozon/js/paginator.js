...
/*____________ПАГИНАТОР, ПЕРЕКЛЮЧЕНИЕ СТРАНИЦ____________*/

let pages = 10;

document.getElementById('pagination').innerHTML = createPagination(pages, 1);

function createPagination(pages, page, name) {
  let str = '<ul>';
let active;
  ...
  if (page > 1) {
    str += '<a class="btn-left"  onclick="createPagination(pages, '+(page-1)+')"></a>';
  }
  ...
  if (page < pages) {
    str += '<a class="btn-right" onclick="createPagination(pages, '+(page+1)+')">';
  }
  str += '</ul>';
